                  ...
                  <div class="row">
                     <div class="col-lg-12">
                        <div class="mb-2">
                           ...
                           <input type="text" name="product_slug" required="required" id="product_slug" class="form-control border-info" value="{{ $data['product']->slug }}">
               ...
               <div class="card-header d-flex justify-content-between align-items-center gap-1">
                  <h4 class="card-title">
                     ...
                     <p class="text-muted mb-0" style="font-size: 12px;">
                        Paste format:<br>
                        <code>Fabric Heavy-Duty PP Woven Fabric</code><br>
                        <code>Capacity 500 Kg</code><br>
               ...
               <div class="card-body add-more-additional-feature-append">
                  @foreach($data['product']->additionalFeatures as $index => $additionalFeature)
                     <div class="row feature-row" id="feature-row-{{ $index }}">
                        <div class="col-lg-5">
                           <div class="mb-2">
                              <input type="text" name="additional_feature_key[]"  id="additional-feature-key-{{ $index }}" class="form-control" value="{{ $additionalFeature->feature->title }}" placeholder="Enter additional feature key" >
                           </div>
                        </div>
                        <div class="col-lg-5">
                           <div class="mb-2">
                              <input type="text" name="additional_feature_key_value[]" id="additional-feature-key-value-{{ $index }}" class="form-control" value="{{ $additionalFeature->product_additional_feature_value }}" placeholder="Enter additional feature key value" >
                           </div>
                        </div>
                        <div class="col-lg-2">
                  ...
                  <div class="row">
                     ...
                     <div class="col-lg-12">
                        <div class="mb-2">
                           ...
                           <textarea class="form-control bg-light-subtle" id="meta_description" rows="2" name="meta_description" placeholder="Short description about meta description">{{ $data['product']->meta_description }}</textarea>
                        </div>
                     </div>
                  </div>
            ...
            <div class="card card_fixed">
               <div class="card-footer bg-light-subtle">
                  <div class="row g-2">
                     ...
                     <div class="col-lg-6">
                        <a class="btn btn-primary w-100" href="{{ route('product.index')}}">Cancel</a>
                     </div>
                  </div>
               </div>
            </div>
...
<script>
   $(document).ready(function() {
      ...
      /* Submit button make disbled when form submit */
      ...
      */
      $(document).ready(function() {
         ...
         $(document).on('paste', '.add-more-additional-feature-append .form-control', function (e) {
            ...
            let currentRow = $(this).closest('.feature-row');
            lines.forEach((line, index) => {
               ...
               if (index === 0) {
                     fillRow(currentRow, key, value);
               } else {
                     let nextRow = currentRow.next('.feature-row');
                     if (nextRow.length) {
                        currentRow = nextRow;
                        fillRow(currentRow, key, value);
                     } else {
                        let lastRow = $('.feature-row').last();
                        let newIndex = lastRow.length ? parseInt(lastRow.attr('id').split('-')[2]) + 1 : 0;
                        let newRow = $(`
                           <div class="row feature-row" id="feature-row-${newIndex}">
                                 <div class="col-lg-5">
                                    <div class="mb-2">
                                       <input type="text" name="additional_feature_key[]" class="form-control" id="additional-feature-key-${newIndex}" placeholder="Enter additional feature key">
                                    </div>
                                 </div>
                                 <div class="col-lg-5">
                                    <div class="mb-2">
                                       <input type="text" id="additional-feature-key-value-${newIndex}" name="additional_feature_key_value[]" class="form-control" placeholder="Enter additional feature key value">
                                    </div>
                                 </div>
                                 <div class="col-lg-2">
                                    <button type="button" class="btn btn-danger remove-feature btn-sm" data-id="${newIndex}">Remove</button>
                                 </div>
                           </div>
                        `);

                        $('.add-more-additional-feature-append').append(newRow);
            ...
            function fillRow(row, key, value) {
               let inputs = row.find('.form-control');
               inputs.eq(0).val(key);
