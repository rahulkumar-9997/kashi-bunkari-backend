         ...
         <div class="card">
            ...
            <div class="card-body">
                        ...
                        <tr>
                           ...
                           <td>
                              ...
                              <a
                                 ...
                                 <span class="badge bg-danger text-light border py-1 px-2 mb-1">
                                    {{ $attribute->title }}
                              ...
                              <a
                                 ...
                                 href="{{ route('update-hsn-gst-with-attributes-value', ['attributes_id' => $attribute->id, 'category_id' => $category_list_row->id]) }}">
                                 <span class="badge bg-info-subtle text-muted border py-1 px-2 mb-1">
                                    {{ $attribute->title }}
                                 </span>
                              </a>
                              @else
                              <!-- Inactive Link -->
                              <span class="badge bg-dark border py-1 px-2" data-bs-toggle="tooltip" title="This attribute is not mapped mb-1">
                                 {{ $attribute->title }}
                              </span>
                              @endif
                              @endforeach
                           ...
                           <td>
                              ...
                              <br><span class="badge border border-success text-success">Trending Yes</span>
                              @else
                              <br><span class="badge border border-danger text-danger">Trending No</span>
                              @endif
                              
                           </td>
                           <td>
                              @php
                                 $minSort = $data['category_list']->min('sort_order');
                                 $maxSort = $data['category_list']->max('sort_order');
                              @endphp
                              <div class="d-flex gap-2 align-items-center">
                                 <form method="POST" action="{{ route('category.sort') }}">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $category_list_row->id }}">
                                    <input type="hidden" name="direction" value="up">
                                    <button type="submit"
                                          class="btn btn-sm btn-light"
                                          @disabled($category_list_row->sort_order == $minSort)>
                                 ...
                                 <form method="POST" action="{{ route('category.sort') }}">
                                    ...
                                    <button type="submit"
                                          class="btn btn-sm btn-light"
                                          @disabled($category_list_row->sort_order == $maxSort)>
               ...
               <div class="my-pagination mt-2" style="float:right;">
                  {{ $data['category_list']->links('vendor.pagination.bootstrap-4') }}
               </div>
               @endif
            </div>
         </div>
