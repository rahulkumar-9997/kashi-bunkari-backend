...
class ProductsController extends Controller
{
                                ...
                                if (!empty($additional_feature_key_value_array[$key])) {
                                    $product_additional_feature = ProductsAdditionalFeature::create(
                                        [
                                            ...
                                            'product_additional_feature_value' => $additional_feature_key_value_array[$key],
                                            'sort_order' => $key
                                        ]
                                    );
    ...
    public function update(Request $request, $id) {
        ...
        */
        ...
        try {
            ...
            $input['product_tags'] = $request->input('product_tags') ?? null;  
            ...
            $input['product_short_description'] = $request->input('product_short_description');
            ...
            if (!empty($request->input('additional_feature_key'))){
                if ($request->has('additional_feature_key')) {
                    $keys   = $request->input('additional_feature_key');
                    $values = $request->input('additional_feature_key_value');
                    ProductsAdditionalFeature::where('product_id', $update_product_row->id)->delete();
                    foreach ($keys as $index => $key) {
                        $key   = trim($key);
                        $value = isset($values[$index]) ? trim($values[$index]) : null;
                        if (empty($key) || empty($value)) {
                            continue;
                        }
                        $feature = AdditionalFeature::firstOrCreate(
                            ['title' => $key],
                            ['slug' => Str::slug($key)]
                        );
                        ProductsAdditionalFeature::create([
                            'product_id' => $update_product_row->id,
                            'additional_feature_id' => $feature->id,
                            'product_additional_feature_value' => $value,
                            'sort_order' => $index
                        ]);
                    }
                }
    ...
    public function productMultipleUpdatePage(Request $request)
    {
        ...
        try {
            $query = Product::with(['images', 'category'])
                ->select(['id', 'title', 'category_id', 'meta_title', 'meta_description', 'g_tin_no', 'slug', 'category_id', 'product_short_description', 'product_description', 'product_specification', 'video_id', 'length', 'breadth', 'height', 'weight', 'volumetric_weight_kg']); 
    ...
    public function productMultipleUpdatePageSubmit(Request $request) {
        ...
        foreach ($products as $key => $productId) {
            ...
            switch ($criteria) {
                ...
                case 'product-short-description':
                    if (isset($request->product_short_description[$key])) {
                        $rules["product_short_description.{$key}"] = 'nullable|string';
                    }
                    break;
        ...
        try {
            foreach ($products as $key => $productId) {
                ...
                if ($criteria === 'product-short-description' && isset($request->product_short_description[$key])) {
                    Log::info("Updating product short description for product ID: {$productId}.");
                    $product->product_short_description = trim($request->product_short_description[$key]);
                    $updated = true;
                    $msg ="Product short description updated successfully.";
                }
    ...
    public function makeDuplicateProduct(Request $request, $id)
    {
        try {
            DB::beginTransaction();
            $originalProduct = Product::with(['images', 'category', 'brand', 'attributes.attribute', 'attributes.values.attributeValue', 'additionalFeatures.feature'])->findOrFail($id);
            //Log::info('Original Product Complete Data: ' . json_encode($originalProduct, JSON_PRETTY_PRINT));
            //Log::info('Starting product duplication process', ['original_product_id' => $id]);
            $newProductData = [
                'title' => $originalProduct->title . ' Duplicate', 
                'category_id' => $originalProduct->category_id,
                'hsn_code' => $originalProduct->hsn_code,
                'gst_in_per' => $originalProduct->gst_in_per,
                'label_id' => $originalProduct->label_id,
                'product_stock_status' => $originalProduct->product_stock_status,
                'product_tags' => $originalProduct->product_tags,
                'product_status' => 0,
                'warranty_status' => $originalProduct->warranty_status,
                'attributes_show_status' => $originalProduct->attributes_show_status,
                ...
                'product_specification' => $originalProduct->product_specification,
                'length' => $originalProduct->length,
                'breadth' => $originalProduct->breadth,
                'height' => $originalProduct->height,
                'weight' => $originalProduct->weight,
                'volumetric_weight_kg' => $originalProduct->volumetric_weight_kg,
            ];
            Log::info('Creating duplicate product with data:', ['duplicate_data' => $newProductData]);
            $newProduct = Product::create($newProductData);            
            /* Attributes duplicate */
            foreach ($originalProduct->attributes as $attribute) {
                $newAttribute = ProductAttributes::create([
                ...
                foreach ($attribute->values as $value) {
                    ProductAttributesValues::create([
                        ...
                        'attributes_value_id' => $value->attributes_value_id,
                        'sort_order' => $value->sort_order,
                    ]);
                }
            }            
            /* Additional features duplicate */
            $featureCount = 0;
            if($originalProduct->additionalFeatures && $originalProduct->additionalFeatures->count() > 0){
                foreach ($originalProduct->additionalFeatures as $feature) {
                    ProductsAdditionalFeature::create([
                        'product_id' => $newProduct->id,
                        'additional_feature_id' => $feature->additional_feature_id,
                        'product_additional_feature_value' => $feature->product_additional_feature_value,
                        'sort_order' => $feature->sort_order,
                    ]);
                    $featureCount++;
                }
            }
            //Log::info('Additional features duplicated', ['total_features' => $featureCount]);            
            /* DUPLICATION RECORD STORE  */
            ProductDuplication::create([
